        ...
        for (byte[] command : commands) {
            ...
            switch (aRequest.getType()) {
                case CREATE: { // ##### operation: create #####
                    // Only create records
                    if (Objects.requireNonNull(aRequest.getEntity()) == YCSBMessage.Entity.RECORD) { // ##### entity: record #####
...
//		System.out.println("RETURNING REPLY");
    ...
    public byte[] appExecuteUnordered(byte[] theCommand, MessageContext theContext) {
        ...
        if (Objects.requireNonNull(aRequest.getType()) == YCSBMessage.Type.READ) {// ##### operation: read #####
