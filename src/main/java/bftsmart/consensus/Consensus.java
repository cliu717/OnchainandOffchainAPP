...
public class Consensus {
    ...
    private final ExecutionManager manager; // Execution manager for this replica's consensus instances

    private final Decision decision; // Decision instance to which this consensus works for
    private final HashMap<Integer,Epoch> epochs = new HashMap<Integer,Epoch>(2);
    private final ReentrantLock epochsLock = new ReentrantLock(); // Lock for concurrency control
    private final ReentrantLock writeSetLock = new ReentrantLock(); //lock for this consensus write set
    ...
    private final HashSet<TimestampValuePair> writeSet = new HashSet<TimestampValuePair>();
    ...
    public void removeEpochs(int limit) {
        ...
        for(Integer key : epochs.keySet().toArray(new Integer[0])) {
            if(key > limit) {
