...
public class Configuration {
	...
	protected void init() {
		...
		try {
			...
			String s = configs.remove("system.autoconnect");
			if (s == null) {
				autoConnectLimit = -1;
			} else {
			...
			if (s == null) {
				...
				channelsBlocking = s.equalsIgnoreCase("true");
			}

			s = configs.remove("system.communication.secretKeyAlgorithm");
			if (s == null) {
			...
			s = configs.remove("system.communication.defaultKeyLoader");
			if (s == null) {
