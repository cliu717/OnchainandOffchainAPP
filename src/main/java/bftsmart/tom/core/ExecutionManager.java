...
public final class ExecutionManager {
    ...
    private final Acceptor acceptor; // Acceptor role of the PaW algorithm
    private final Proposer proposer; // Proposer role of the PaW algorithm
    ...
    //******* EDUARDO END **************//
    private final Map<Integer, Consensus> consensuses = new TreeMap<Integer, Consensus>(); // Consensuses
    private final ReentrantLock consensusesLock = new ReentrantLock(); //lock for consensuses table
    // Paxos messages that were out of context (that didn't belong to the consensus that was/is is progress
    private final Map<Integer, List<ConsensusMessage>> outOfContext = new HashMap<Integer, List<ConsensusMessage>>();
    // Proposes that were out of context (that belonged to future consensuses, and not the one running at the time)
    private final Map<Integer, ConsensusMessage> outOfContextProposes = new HashMap<Integer, ConsensusMessage>();
    private final ReentrantLock outOfContextLock = new ReentrantLock(); //lock for out of context
    ...
    private final Queue<ConsensusMessage> stoppedMsgs = new LinkedList<ConsensusMessage>();
    private Epoch stoppedEpoch = null; // epoch at which the current consensus was stopped
    private final ReentrantLock stoppedMsgsLock = new ReentrantLock(); //lock for stopped messages
    private TOMLayer tomLayer; // TOM layer associated with this execution manager
    private final int paxosHighMark; // Paxos high mark for consensus instances
    
    /** THIS IS JOAO'S CODE, TO HANDLE THE STATE TRANSFER */
    
    private final int revivalHighMark; // Paxos high mark for consensus instances when this replica CID equals 0
    private final int timeoutHighMark; // Paxos high mark for a timed-out replica
     ...
     */
    public boolean checkLimits(ConsensusMessage msg) {
